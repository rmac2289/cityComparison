...
export default function TabCategories(props) {
    ...
    const data = salaries.map(v => {
        ...
        const slicedLow = lowToStr[5] === "." ? lowToStr.slice(0,5):lowToStr.slice(0,6);
        return {
            "name": v.job.title,
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active) {
            ...
            lowArray.length === 5 ? lowArray.splice(2, 0, ','):lowArray.splice(3, 0, ',')
            highArray.length === 5 ? highArray.splice(2, 0, ','): highArray.splice(3, 0, ',')
            lowPayloadFormatted = lowArray.join('')
            highPayloadFormatted = highArray.join('')    
          return (
            <div className="custom-tooltip">
              <p className="label-1">{label}</p>
              <p className="intro-1">{payload[0].name}: ${lowPayloadFormatted} - ${highPayloadFormatted}</p>
            </div>
          );
        }
        return null;
            ...
            <div className="salaries">
                ...
                <ResponsiveContainer  width='100%' minHeight={1600}>
                    <BarChart margin={{top: 0}} barCategoryGap="15%" height={1600} data={data} layout="vertical">
                        <Tooltip offset={0} wrapperStyle={{ top: -75, left: 100, width: "250px" }} content={<CustomTooltip/>} />
